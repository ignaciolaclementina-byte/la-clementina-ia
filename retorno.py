...
<style>
    ...
    /* Efecto Hover y Sombra en Tarjetas */
    .card-white, .card-vip, .card-cosecha { transition: all 0.3s ease-in-out; cursor: pointer; }
    ...
    .card-vip { background: #fff9e6 !important; border: 3px solid #f1c40f !important; border-radius: 15px; padding: 20px; margin-bottom: 15px; color: #333; box-shadow: 0px 4px 15px rgba(241, 196, 15, 0.3); }
    .card-cosecha { background: #e8f5e9 !important; border: 2px solid #2e7d32 !important; border-radius: 15px; padding: 20px; margin-bottom: 15px; color: #1b5e20; min-height: 220px; }
...
with st.container():
    ...
    busqueda_libre = st.text_input("🔎 Búsqueda rápida (Empresa, Localidad, Mercadería...)", "").upper()
...
with tab1:
    ...
    with col_r1:
        with st.spinner("Actualizando cartelera de camiones..."):
            if not df_ch_raw.empty:
                ...
                for _, r in df_f.iterrows():
                    ...
                    if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])) and match_text:
                        ...
                        texto_wsp = f"─── *RETORNO MATCH VIP* ───\n✅ *SOLICITUD DE TRANSPORTE*\n\n📍 *ORIGEN:* {r[1]}\n🏁 *DESTINO:* {r[2]}\n🚛 *EQUIPO:* {r[3]}\n\nHola, me interesa su unidad vista en el sistema. ¿Está disponible? Saludos."
...
with tab2:
    ...
    with col_r2:
        with st.spinner("Buscando cargas disponibles..."):
            if not df_ca_raw.empty:
                ...
                for _, r in df_f2.iterrows():
                    match_text = busqueda_libre in str(r).upper()
                    if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and match_text:
                        # WhatsApp Estructurado
                        texto_wsp_ca = f"─── *RETORNO MATCH VIP* ───\n📦 *INTERÉS EN CARGA*\n\n📍 *TRAYECTO:* {r[1]} a {r[2]}\n📦 *CARGA:* {r[3]}\n🏢 *EMPRESA:* {r[5]}\n\nHola, consulto por la carga vista en el sistema. ¿Sigue disponible?"
    ...
    with col_a1:
        ...
        with st.form("f_arr", clear_on_submit=True):
            ...
            if st.form_submit_button("PUBLICAR ARRIME"):
                st.toast("🌾 Publicando operativo...", icon="🚜")
    ...
    with col_a2:
        if not df_ca_raw.empty:
            ...
            for i, (_, r) in enumerate(df_arrime.iterrows()):
                ...
                msg_arr = f"─── *RETORNO MATCH VIP - ARRIME* ───\n🌾 *COSECHA 2026*\n\n📍 *ZONA:* {r[2]}\n🌾 *DETALLE:* {detalle}\n💰 *TARIFA:* {tarifa}\n\nHola, me interesa el operativo. ¿Cómo procedemos?"
