...
# MEJORA: Limpieza automática de datos obsoletos (más de 24hs)
def filtrar_por_vigencia(df):
    if df.empty: return df
    try:
        limite = ahora - timedelta(hours=24)
        df_temp = df.copy()
        df_temp.iloc[:, 0] = pd.to_datetime(df_temp.iloc[:, 0], dayfirst=True, errors='coerce')
        return df_temp[df_temp.iloc[:, 0] >= limite]
    except: return df

df_ch_raw = filtrar_por_vigencia(df_ch_raw)
df_ca_raw = filtrar_por_vigencia(df_ca_raw)
...
def ocultar_telefono(num, es_vip_user):
    if es_vip_user: return num
    clean = "".join(filter(str.isdigit, str(num)))
...
with st.container():
    ...
    col_bus, col_clear = st.columns([3, 1])
    with col_bus:
        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_f.iterrows():
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])) and (busqueda_libre in str(r).upper()):
                    distancia = calcular_distancia(r[1], r[2])
                    wsp = r[5] if len(str(r[4])) == 11 else r[4]
                    msg_chofer = urllib.parse.quote(f"Hola! Vi tu camión en RETORNO MATCH VIP para la ruta {r[1]} -> {r[2]}. ¿Está disponible?")
                    link_wsp = f"https://api.whatsapp.com/send?phone={limpiar_wsp(wsp)}&text={msg_chofer}"
                    st.markdown(f'<div class="{"card-vip" if r["vip"] else "card-white"}">{f"<span class=\'dist-badge\'>{distancia}</span>" if distancia else ""}{"<div class=\'vip-label\'>⭐ CHOFER VIP</div>" if r["vip"] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛 EQUIPO:</b> {r[3]} | 📱 <b>TEL:</b> {ocultar_telefono(wsp, soy_vip_actual)}<br><a href="{link_wsp}" target="_blank" class="btn-wsp">✉️ ENVIAR PROPUESTA</a></div>', unsafe_allow_html=True)
    ...
    with col_f2:
        ...
        with st.form("f_ch", clear_on_submit=True):
            ...
            if st.form_submit_button("SUBIR CAMIÓN"):
                if validar_cuit(cu_id) and len(wsp_num) > 8:
                    ...
                    time.sleep(1); st.cache_data.clear(); st.rerun()
                else: st.error("Verifique CUIT y WhatsApp")
    with col_r2:
        if not df_ca_raw.empty:
            ...
            for _, r in df_ca_raw.iterrows():
                ...
                if minutos < TIEMPO_EXCLUSIVO_MIN and not soy_vip_actual:
                    ...
                    msg_carga = urllib.parse.quote(f"Hola {r[5]}, vi tu carga de {r[3]} en RETORNO MATCH VIP. ¿Sigue disponible?")
                    link_wsp_ca = f"https://api.whatsapp.com/send?phone={limpiar_wsp(r[4])}&text={msg_carga}"
                    st.markdown(f'<div class="{"card-vip" if r["vip"] else "card-white"}">{f"<span class=\'dist-badge\'>{distancia}</span>" if distancia else ""}{"<div class=\'vip-label\'>⭐ EMPRESA VIP</div>" if r["vip"] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>📦 CARGA:</b> {r[3]} | 🏢 <b>EMPRESA:</b> {r[5]} | 📱 <b>TEL:</b> {ocultar_telefono(r[4], soy_vip_actual)}<br><a href="{link_wsp_ca}" target="_blank" class="btn-wsp">📩 CONSULTAR</a></div>', unsafe_allow_html=True)
    ...
    with col_a2:
        if not df_ca_raw.empty:
            ...
            for _, r in df_arrime.iterrows():
                st.markdown(f'<div class="card-cosecha"><div class="route-txt" style="color:#2e7d32;">📍 {r[2]}</div>{r[3]} | 📱 {ocultar_telefono(r[4], soy_vip_actual)}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[4])}" target="_blank" class="btn-wsp" style="background-color:#2e7d32;">🚜 CONTACTAR</a></div>', unsafe_allow_html=True)
...
<div class="legal-footer">
    ...
    <p><b>Prohibida la copia total o parcial de esta interfaz sin autorización de Ignacio Diaz y sus legales.</b></p>
</div>
""", unsafe_allow_html=True)
