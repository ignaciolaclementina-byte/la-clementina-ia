...
<style>
    ...
    .card-vip { background: #fff9e6 !important; border: 3px solid #f1c40f !important; color: #333; box-shadow: 0px 4px 15px rgba(241, 196, 15, 0.3); border-radius: 15px; padding: 20px; margin-bottom: 15px;}
    .match-tag { background: #f1c40f; color: black; padding: 2px 8px; border-radius: 5px; font-size: 12px; font-weight: bold; float: right; animation: pulse 2s infinite; }
        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_f.iterrows():
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])) and (busqueda_libre in str(r).upper()):
                    # Lógica de Match (Mejora: Si coincide origen y destino buscado)
                    es_match = (b_o != "CUALQUIERA" and b_o in str(r[1]).upper()) and (b_d != "CUALQUIERA" and b_d in str(r[2]).upper())
                    match_badge = '<span class="match-tag">✨ MATCH PERFECTO</span>' if es_match else ""
                    minutos = obtener_minutos_desde_publicacion(r[0])
                    card_class = get_card_style(minutos, r['vip'])
                    st.markdown(f'<div class="{card_class}">{match_badge}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛 EQUIPO:</b> {r[3]} | 📱 {ocultar_telefono(r[5])}<br><a href="https://wa.me/{limpiar_wsp(r[5])}" target="_blank" class="btn-wsp">✉️ ENVIAR PROPUESTA</a></div>', unsafe_allow_html=True)
...
with tab2:
    ...
    with col_f2:
        ...
        with st.form("f_ch", clear_on_submit=True):
            ...
            et, cid, wnu = st.selectbox("Equipo", EQUIPOS[1:]), st.text_input("CUIT"), st.text_input("WhatsApp")
        ...
        if not df_ca_raw.empty:
            ...
            for _, r in df_f2.iterrows():
                ...
                if minutos < TIEMPO_EXCLUSIVO_MIN and not soy_vip_actual:
                    ...
                    # Lógica de Match
                    es_match = (b_o != "CUALQUIERA" and b_o in str(r[1]).upper()) and (b_d != "CUALQUIERA" and b_d in str(r[2]).upper())
                    match_badge = '<span class="match-tag" style="background:#2ecc71; color:white;">⭐ CARGA IDEAL</span>' if es_match else ""
                    txt_w = urllib.parse.quote(f"📢 CARGA: {r[1]} a {r[2]}\n📦 {r[3]}\n✅ Retorno Match VIP")
                    st.markdown(f'<div class="card-medium">{match_badge}<div class="route-txt">{r[1]} ➔ {r[2]}</div>📦 {r[3]} | 🏢 {r[5]}<br><a href="https://wa.me/{limpiar_wsp(r[4])}" class="btn-wsp">📩 CONSULTAR</a></div>', unsafe_allow_html=True)
