...
# --- 2. ESTILOS Y DISEÑO (ESTRUCTURA RECORDADA - SIN CAMBIOS) ---
st.markdown("""
    <style>
    .stApp {
...
with tab_chofer:
    ...
    with col_d:
        ...
        try:
            ...
            df_final = df_c[df_c['fecha_temp'] == fecha_filtro]
            ...
            for _, r in df_final.iloc[::-1].iterrows():
                ...
                estado_c = str(r[7]).upper() # Columna H
                ...
                btn_html = f'<a href="https://api.whatsapp.com/send?phone=549{r[4]}&text={msg_ch}" target="_blank" class="btn-wsp">💬 CONSULTAR CARGA</a>' if not es_concretada else "<b>CARGA FINALIZADA</b>"

                st.markdown(f'<div class="{card_style}">{badge_html}<div class="route-txt">📍 {r[1]} ➔ {r[2]}</div><b>📦 CARGA:</b> {r[3]} | 🏢 <b>EMPRESA:</b> {r[5]}<br><b>⏳ SALE:</b> {r[6]}{btn_html}</div>', unsafe_allow_html=True)
        except: st.info("No hay cargas para esta fecha.")
    ...
    with col_b:
        ...
        try:
            ...
            for _, r in df_h.iloc[::-1].iterrows():
                ...
                estado = str(r[8]).upper().strip() # Columna I
