...
# IDs de Google Forms (Verificados según tu estructura)
...
def cargar_datos_seguros():
    try:
        t = int(time.time())
        # Cargamos los datos asegurando que no fallen por formatos
...
def es_fecha(f, target):
    try:
        # Convertimos la fecha de la planilla al formato de comparación
...
# --- TAB 1: CAMIONES (EMPRESA CONTACTA AL CHOFER) ---
with t1:
    ...
    with col_r1:
        if not df_ch_raw.empty:
            ...
            # Filtramos por fecha y ordenamos por VIP
            df_f = df_ch_raw[df_ch_raw.iloc[:, 0].apply(lambda x: es_fecha(x, b_fecha))].sort_values(by='vip', ascending=False)
            
            for _, r in df_f.iterrows():
                # Filtros de búsqueda
                cond_o = (b_o=="CUALQUIERA" or b_o in str(r[1]).upper())
                cond_d = (b_d=="CUALQUIERA" or b_d in str(r[2]).upper())
                cond_e = (b_e=="CUALQUIERA" or b_e==str(r[3]))
                
                if cond_o and cond_d and cond_e:
                    ...
                    texto_wsp = (
                        ...
                        f"Hola, me interesa su unidad:\n"
                        f"📍 *ORIGEN:* {r[1]}\n"
                        f"🏁 *DESTINO:* {r[2]}\n"
                        f"🚛 *EQUIPO:* {r[3]}\n\n"
                        f"¿Podría confirmarme disponibilidad? Gracias."
                    )
                    ...
                    <div class="{"card-vip" if r["vip"] else "card-white"}">
                        ...
                        <a href="{link_wsp}" target="_blank" class="btn-wsp">✉️ ENVIAR PROPUESTA</a>
            ...
            for _, r in df_f2.iterrows():
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()):
                    texto_wsp_ca = (
                        f"─── *RETORNO MATCH VIP* ───\n"
                        f"📦 *INTERÉS EN CARGA*\n\n"
                        f"Hola, consulto por la carga:\n"
                        f"🏢 *EMPRESA:* {r[5]}\n"
                        f"📦 *MERCADERÍA:* {r[3]}\n"
                        f"📍 *RUTA:* {r[1]} ➔ {r[2]}\n\n"
                        f"¿Sigue disponible? Gracias."
                    )
