...
<style>
    ...
    .btn-share {
        ...
        border-radius: 8px; text-decoration: none; font-size: 12px; display: inline-block; margin-top: 5px; text-align: center;
    }
...
# --- 4. BÚSQUEDA MEJORADA (FILTRO EQUIPO) ---
        ...
        try:
            ...
            for _, r in df_ca.iloc[::-1].iterrows():
                if es_hoy(r[0]) and (b_o == "CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d == "CUALQUIERA" or b_d in str(r[2]).upper()):
                    ...
                    share_msg = urllib.parse.quote(f"¡Carga disponible en Retorno Match!\n📍 Origen: {r[1]}\n🏁 Destino: {r[2]}\n📦 Carga: {r[3]}\n\nConsultala acá: [Link de tu App]")
                    ...
                    st.markdown(f'''<div class="{"card-urgent" if urg else "card-white"}">
                        ...
                        <a href="https://api.whatsapp.com/send?text={share_msg}" target="_blank" class="btn-share">🔗 Compartir con un colega</a>
                    </div>''', unsafe_allow_html=True)
        except: st.info("Buscando cargas del día...")
    ...
    with col_r2:
        try:
            ...
            for _, r in df_ch.iloc[::-1].iterrows():
                # Aplicamos el Filtro de Equipo aquí
        ...
        except: st.info("Buscando camiones del día...")
