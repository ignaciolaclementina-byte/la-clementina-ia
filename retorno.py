...
# --- 4. CONFIGURACIÓN E INTERFAZ PREMIUM ---
...
<style>
    ...
    .legal-footer { text-align: center; color: white; padding: 40px; margin-top: 50px; opacity: 0.8; }
</style>
<a href="https://api.whatsapp.com/send?text=¡Encontré esta App de Retornos! 🚚 https://retornomatchvip.streamlit.app" class="share-btn" target="_blank">📲</a>
""", unsafe_allow_html=True)

# --- 5. FUNCIONES DE LIMPIEZA ---
def sanitizar(dato):
    s = "".join(filter(str.isdigit, str(dato).replace(".0", "")))
    return s
...
b_q = st.text_input("📍 BUSCADOR RÁPIDO (Escribe ciudad, empresa o carga...)", placeholder="Ej: San Jorge, Arcor, Soja...").upper()
...
radar_txt = f"🔥 HOY: {cant_ch} Camiones y {cant_ca} Cargas -- ⭐ {st.session_state.anuncios} -- Creado por Ignacio Diaz."
...
with t1:
    ...
    with f1:
        ...
        with st.form("f_ca", clear_on_submit=True):
            eo = st.selectbox("Origen", PROVINCIAS[1:]); elo = st.text_input("Loc.")
            ed = st.selectbox("Destino", PROVINCIAS[1:]); eld = st.text_input("Loc.")
            ec = st.text_input("Carga"); en = st.text_input("Empresa"); ew = st.text_input("WhatsApp (Ej: 1123456789)")
    ...
    with r1:
        if not df_ch_raw.empty:
            ...
            for _, r in df_f.iterrows():
                content = f"{r[1]} {r[2]} {r[3]} {r[4]} {r[5]}".upper()
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])) and (b_q in content):
...
with t2:
    ...
    with f2:
        ...
        with st.form("f_ch", clear_on_submit=True):
            o_p = st.selectbox("Prov. Origen", PROVINCIAS[1:]); o_l = st.text_input("Loc.")
            d_p = st.selectbox("Prov. Destino", PROVINCIAS[1:]); d_l = st.text_input("Loc.")
    ...
    with r2:
        if not df_ca_raw.empty:
            ...
            for _, r in df_f2.iterrows():
                content_ca = f"{r[1]} {r[2]} {r[3]} {r[5]}".upper()
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_q in content_ca):
                    badge_ca = '<div class="badge-nuevo">NUEVO</div>' if es_nuevo(r[0]) else ''
                    msg_ca = urllib.parse.quote(f"─── *RETORNO MATCH VIP* ───\n📦 *INTERÉS EN CARGA*\n🏢 EMPRESA: {r[5]}\n📍 RUTA: {r[1]} -> {r[2]}\n\n¿Sigue disponible?")
                    st.markdown(f'''<div class="{"card-vip" if r["vip"] else "card-white"}">{badge_ca}
...
# --- PIE DE PÁGINA ---
st.markdown(f'<div class="legal-footer"><b>Creado por Ignacio Diaz</b><br>Prohibida la copia total o parcial. © 2026 RETORNO MATCH VIP</div>', unsafe_allow_html=True)
