            ...
            if st.form_submit_button("PUBLICAR"):
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        st.error(f"❌ Error {res.status_code}. Revisá que el Form sea PÚBLICO.")
            ...
            if st.form_submit_button("SUBIR CARGA"):
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        st.error(f"❌ Error {res.status_code}. Revisá el Form de Cargas.")
                except Exception as ex:
                    st.error(f"❌ Fallo de conexión: {ex}")
    ...
    with col_b:
        ...
        try:
            ...
            for _, r in df_h.iloc[::-1].iterrows():
                ...
                st.markdown(f'<div class="card-white">{badge}<div class="route-txt">🚛 {r[1]} ➔ {r[2]}</div><b>⚙️ EQUIPO:</b> {r[3]}<br><b>🆔 CUIT:</b> {r[5]}<div style="display:flex;gap:10px;"><a href="https://api.whatsapp.com/send?phone=549{r[4]}" target="_blank" class="btn-wsp" style="flex:2;">💬 HABLAR</a><a href="{r[7]}" target="_blank" class="btn-wsp" style="background:#3498db; flex:1;">📂 PAPELES</a></div></div>', unsafe_allow_html=True)
        except: st.info("Buscando...")

# --- PANEL ADMIN Y FOOTER (Igual que antes) ---
st.markdown("---")
with st.expander("🛠️ PANEL DE ADMINISTRACIÓN"):
    pw = st.text_input("Introduce la Clave Maestra", type="password")
    if pw == ADMIN_PASSWORD:
        st.link_button("📂 EXCEL: CHOFERES", f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/edit#gid={GID_CHOFERES}")
        st.link_button("🗑️ EXCEL: CARGAS", f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/edit#gid={GID_CARGAS}")

st.markdown(f'<div class="footer"><p><b>© 2026 RETORNO MATCH - Ignacio Diaz</b></p></div>', unsafe_allow_html=True)
