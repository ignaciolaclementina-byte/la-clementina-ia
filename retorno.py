...
GID_VIP = "968995524" # Pestaña VIP vinculada para impacto global
...
# Combinamos con los VIPs locales de la sesión (por si agregas desde el panel de la app)
if 'socios_activos' not in st.session_state:
    st.session_state.socios_activos = "TRANSPORTES SAN JORGE, LOGISTICA DIAZ"
...
def es_vip(dato):
    ...
    lista_app = [s.strip().upper() for s in st.session_state.socios_activos.split(",") if s.strip()]
    return dato_str in LISTA_VIPS_GLOBAL or dato_str in lista_app
        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_final_ch.iterrows():
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])):
                    ...
                    cuit_v = limpiar_dato_numerico(r[4]); wsp_v = limpiar_dato_numerico(r[5])
                    wsp_final = wsp_v if len(cuit_v) == 11 else cuit_v
                    st.markdown(f'<div class="{clase}">{label}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛:</b> {r[3]} | 🆔: {cuit_v}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(wsp_final)}&text=Hola!" target="_blank" class="btn-wsp">💬 CONTACTAR POR WHATSAPP</a></div>', unsafe_allow_html=True)
        ...
        if not df_ca_raw.empty:
            ...
            for _, r in df_final_ca.iterrows():
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()):
                    ...
                    st.markdown(f'<div class="{clase}">{label}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>📦:</b> {r[3]} | 🏢: {r[5]}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[4])}&text=Hola!" target="_blank" class="btn-wsp">💬 CONSULTAR CARGA</a></div>', unsafe_allow_html=True)
