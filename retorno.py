...
def es_fecha_seleccionada(f, fecha_target):
    try: return pd.to_datetime(f, dayfirst=True).date() == fecha_target
    except: return False
...
st.markdown(f'<div class="radar-container"><marquee scrollamount="8">🚛 FECHA: {b_fecha.strftime("%d/%m/%Y")} -- ACTIVOS: {cant_camiones} -- ⭐ {st.session_state.anuncios} -- Creado por Ignacio Diaz y sus legales.</marquee></div>', unsafe_allow_html=True)
...
with t1:
    ...
    with col_f1:
        ...
        with st.form("form_carga", clear_on_submit=True):
            ...
            ew = st.text_input("WhatsApp (Sin 0 ni 15)")
            if ew and len("".join(filter(str.isdigit, ew))) >= 10: st.success("✅ WhatsApp Ok")
...
with t2:
    ...
    with col_f2:
        ...
        with st.form("form_camion", clear_on_submit=True):
            ...
            e = st.selectbox("Equipo", EQUIPOS[1:]); cu = st.text_input("CUIT/ID (Sin guiones)")
            if cu and len("".join(filter(str.isdigit, cu))) == 11: st.success("✅ CUIT Ok")
            w = st.text_input("WhatsApp (Sin 0 ni 15)")
            if w and len("".join(filter(str.isdigit, w))) >= 10: st.success("✅ WhatsApp Ok")
    ...
    with col_r2:
        if not df_ca_raw.empty:
            df_ca_raw['es_vip'] = df_ca_raw.iloc[:, 4].apply(es_vip) # VIP basado en nombre empresa (columna 4)
            ...
            for _, r in df_final_ca.iterrows():
                if (b_o == "CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d == "CUALQUIERA" or b_d in str(r[2]).upper()):
                    clase = "card-vip" if r['es_vip'] else "card-white"
                    # CORRECCIÓN: r[4] es Empresa, r[6] es WhatsApp
                    st.markdown(f'''<div class="{clase}">{"<div class='vip-label'>⭐ EMPRESA VIP</div>" if r['es_vip'] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>📦 CARGA:</b> {r[3]} | 🏢 <b>EMPRESA:</b> {r[4]}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[5])}" target="_blank" class="btn-wsp">💬 CONSULTAR</a></div>''', unsafe_allow_html=True)

# --- 8. PANEL DE CONTROL (ELIMINAR VIP RÁPIDO) ---
st.markdown("---")
with st.expander("⚙️ PANEL DE CONTROL (ADMIN)"):
    st.markdown("### ⭐ Gestión Directa de Socios VIP")
    lista_socios = [s.strip() for s in st.session_state.socios_activos.split(",") if s.strip()]
    
    new_vip = st.text_input("Agregar CUIT o Empresa VIP:")
    if st.button("➕ AGREGAR"):
        if new_vip and new_vip not in lista_socios:
            lista_socios.append(new_vip); st.session_state.socios_activos = ", ".join(lista_socios); st.rerun()

    st.write("**Lista VIP Actual (Elimina rápido aquí):**")
    for socio in lista_socios:
        c_s1, c_s2 = st.columns([3, 1])
        with c_s1: st.code(socio)
        with c_s2: 
            if st.button(f"🗑️", key=f"del_{socio}"):
                lista_socios.remove(socio); st.session_state.socios_activos = ", ".join(lista_socios); st.rerun()

# --- 9. PIE DE PÁGINA ---
st.markdown(f'<div class="legal-footer"><p style="font-size: 18px; font-weight: bold; color: white;">Creado por Ignacio Diaz y sus legales</p><p>© 2026 RETORNO MATCH VIP</p></div>', unsafe_allow_html=True)
