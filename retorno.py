...
<style>
    ...
    .card-white { background: white !important; border-radius: 15px; padding: 20px; margin-bottom: 15px; border-left: 10px solid #3498db; color: #333; transition: 0.3s; }
...
with st.container():
    ...
    # Búsqueda por texto libre (Mejora 2)
        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_f.iterrows():
                ...
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and \
                   ...
                   (b_e=="CUALQUIERA" or b_e==str(r[3])) and match_text:
    ...
    with col_r2:
        if not df_ca_raw.empty:
            ...
            for _, r in df_f2.iterrows():
                match_text = busqueda_libre in str(r).upper()
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and \
                   (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and match_text:
...
with tab3:
    ...
    with col_a2:
        if not df_ca_raw.empty:
            ...
            # MEJORA 3: DISEÑO EN COLUMNAS PARA COSECHA
            ...
            for i, (_, r) in enumerate(df_arrime.iterrows()):
                ...
                with cols_arrime[i % 2]: # Distribuye en las 2 columnas
                    st.markdown(f'''
