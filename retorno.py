...
WSP_VENTAS_VIP = "5493401525621" # Contacto comercial VIP
...
with st.container():
    ...
    with c_log1:
        ...
        if user_cuit and not validar_cuit(user_cuit): st.error("⚠️ CUIT no válido.")
        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_f.iterrows():
                ...
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])) and (busqueda_libre in str(r).upper()):
                    ...
                    card_class = "card-vip" if r['vip'] else ("card-hot" if minutos < 60 else "card-white")
                    st.markdown(f'<div class="{card_class}">{"<div class=\'vip-label\'>⭐ CHOFER VIP</div>" if r["vip"] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛 EQUIPO:</b> {r[3]} | 🆔 <b>CUIT:</b> {cuit} | 📱 <b>TEL:</b> {ocultar_telefono(wsp)}<br><a href="{link_wsp}" target="_blank" class="btn-wsp">✉️ ENVIAR PROPUESTA</a></div>', unsafe_allow_html=True)
        ...
        if not df_ca_raw.empty:
            ...
            for _, r in df_f2.iterrows():
                ...
                if minutos < TIEMPO_EXCLUSIVO_MIN and not soy_vip_actual:
                    ...
                    card_class = "card-vip" if r['vip'] else ("card-hot" if minutos < 60 else "card-white")
                    st.markdown(f'<div class="{card_class}">{"<div class=\'vip-label\'>⭐ EMPRESA VIP</div>" if r["vip"] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>📦 CARGA:</b> {r[3]} | 🏢 <b>EMPRESA:</b> {r[5]}<br><a href="{link_wsp_ca}" target="_blank" class="btn-wsp">📩 CONSULTAR</a></div>', unsafe_allow_html=True)
        ...
        if not df_ca_raw.empty:
            ...
            for i, (idx, r) in enumerate(df_arrime.iterrows()):
                with cols_arr[i % 2]:
                    ...
                    if st.session_state.get('admin_mode', False) and st.button(f"🗑️ BORRAR #{i}", key=f"del_{idx}"):
                        requests.post(URL_CARGAS_POST, data={"entry.610070407": "BORRADO", "entry.576675281": f"REF:{r[0]}", "entry.1930562861": "SISTEMA"})
                        st.cache_data.clear(); st.rerun()
