...
<style>
    ...
    .btn-borrar { background-color: #d63031; color: white !important; padding: 8px; border-radius: 10px; text-decoration: none; font-weight: bold; display: block; text-align: center; margin-top: 10px; border: none; width: 100%; cursor: pointer; }
...
with st.container():
    ...
    with c_log1:
        ...
        soy_vip_actual = es_vip(user_cuit) if not user_cuit or validar_cuit(user_cuit) else False
        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_f.iterrows():
                ...
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])) and (busqueda_libre in str(r).upper()):
                    ...
                    st.markdown(f'<div class="{card_class}"><div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛 EQUIPO:</b> {r[3]} | 📱 <b>TEL:</b> {ocultar_telefono(wsp)}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(wsp)}" target="_blank" class="btn-wsp">✉️ ENVIAR PROPUESTA</a></div>', unsafe_allow_html=True)
        ...
        if not df_ca_raw.empty:
            ...
            for i, r in df_f2.iterrows():
                ...
                if minutos < TIEMPO_EXCLUSIVO_MIN and not soy_vip_actual:
                    st.markdown(f'<div class="card-bloqueada">🔒 CARGA EXCLUSIVA VIP<br><a href="https://api.whatsapp.com/send?phone={WSP_VENTAS_VIP}" target="_blank" style="color:#f1c40f;">⭐ ACTIVAR VIP</a></div>', unsafe_allow_html=True)
