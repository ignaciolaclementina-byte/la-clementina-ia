...
# --- 2. ESTILOS (Mantenemos tu base con agregados para concretadas) ---
st.markdown("""
    <style>
    .stApp {
    ...
    .card-concretada {
        background: #f8f9fa !important; border-radius: 15px; padding: 20px; margin-bottom: 15px;
        border-left: 10px solid #95a5a6; color: #7f8c8d; opacity: 0.8;
    }
    .badge-concretada { background: #95a5a6; color: white; padding: 4px 10px; border-radius: 20px; float: right; font-weight: bold; }
...
with c_fecha:
    # Filtro de fecha para buscar viejas
    filtro_fecha = st.date_input("📅 FECHA CARGAS:", datetime.now())
with c_act:
...
with tab_chofer:
    ...
    with col_i:
        ...
        with st.form("form_ch", clear_on_submit=True):
            ...
            if st.form_submit_button("PUBLICAR"):
                data_ch = {"entry.1304806144": o, "entry.1519265625": d, "entry.597193898": e, "entry.1542650763": cuit, "entry.1837643722": linti, "entry.769375120": link_doc, "entry.1574172378": w}
    ...
    with col_d:
        ...
        try:
            ...
            # Convertir la primera columna (Marca temporal de Google) a fecha para filtrar
            df_c['fecha_dt'] = pd.to_datetime(df_c.iloc[:, 0]).dt.date
            
            # Filtrar por fecha seleccionada
            df_filtrado = df_c[df_c['fecha_dt'] == filtro_fecha]

            for _, r in df_filtrado.iloc[::-1].iterrows():
                ...
                # Chequeo de Carga Concretada (Columna H -> índice 7)
                estado_carga = str(r[7]).upper()
                es_concretada = "CONCRETADA" in estado_carga or "CERRADA" in estado_carga
                
                card_class = "card-concretada" if es_concretada else "card-white"
                badge_html = '<div class="badge-concretada">✅ CONCRETADA</div>' if es_concretada else ""
                ...
                st.markdown(f'''
                    <div class="{card_class}">
                        ...
                        <b>⏳ SALE:</b> {r[6]}
                        {"<a href='https://api.whatsapp.com/send?phone=549"+str(r[4])+"&text="+msg_ch+"' target='_blank' class='btn-wsp'>💬 CONSULTAR CARGA</a>" if not es_concretada else "<i>Esta carga ya no está disponible.</i>"}
                    </div>
                ''', unsafe_allow_html=True)
            if len(df_filtrado) == 0: st.warning("No hay cargas para la fecha seleccionada.")
