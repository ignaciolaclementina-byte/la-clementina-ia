...
GID_VIP = "968995524" # Conexión global con tu pestaña VIP
...
def cargar_datos_blindados():
    try:
        ...
        # Lectura global de VIPs desde tu Excel
...
df_ch, df_ca, LISTA_VIPS = cargar_datos_blindados()

# --- 3. INTERFAZ (RESURRECCIÓN DE ESTRUCTURA ORIGINAL) ---
...
st.markdown(f'<div class="radar-container"><marquee scrollamount="8">⭐ Membresías VIP: Ignacio Diaz -- Creado por Ignacio Diaz y sus legales.</marquee></div>', unsafe_allow_html=True)

t1, t2 = st.tabs(["🚀 VER CAMIONES", "🏢 VER CARGAS"])

# --- TAB 1: CAMIONES (ESTRUCTURA 1, 2.2) ---
with t1:
            ...
            for _, r in df_ch.iterrows():
                try:
                    if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()):
                        # VIP GLOBAL: Chequea contra la lista de la pestaña Excel
                        is_v = any(v in str(r[4]).upper() for v in LISTA_VIPS)
                        clase = "card-vip" if is_v else "card-white"
                        label = '<div class="vip-label">⭐ CHOFER VIP</div>' if is_v else ""
                        st.markdown(f'<div class="{clase}">{label}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛:</b> {r[3]} | 🆔: {r[4]}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[5])}&text=Hola!" target="_blank" class="btn-wsp">💬 WHATSAPP</a></div>', unsafe_allow_html=True)
                except: continue

# --- TAB 2: CARGAS (ESTRUCTURA 1, 2.2) ---
with t2:
            ...
            for _, r in df_ca.iterrows():
                try:
                    if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()):
                        ...
                        is_v = any(v in str(r[4]).upper() for v in LISTA_VIPS)
                        clase = "card-vip" if is_v else "card-white"
                        label = '<div class="vip-label">⭐ EMPRESA VIP</div>' if is_v else ""
                        st.markdown(f'<div class="{clase}">{label}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>📦:</b> {r[3]} | 🏢: {r[4]}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[5])}&text=Hola!" target="_blank" class="btn-wsp">💬 WHATSAPP</a></div>', unsafe_allow_html=True)
                except: continue

# --- PIE DE PÁGINA (MENSAJE LEGAL BLINDADO) ---
