...
# --- 4. FUNCIONES AUXILIARES (CON BLINDAJE DE DATOS - IGNACIO DIAZ) ---
...
<style>
    ...
    .btn-pdf { background-color: #e74c3c; color: white !important; padding: 10px; border-radius: 10px; text-decoration: none; font-weight: bold; display: block; text-align: center; margin-top: 5px; font-size: 12px; }
...
radar_txt = f"🌾 COSECHA ACTIVA: {len(df_ch_raw)} Camiones y {len(df_ca_raw)} Cargas -- Creado por Ignacio Diaz."
        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_f.iterrows():
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])) and (busqueda_libre in str(r).upper()):
                    ...
                    st.markdown(f'<div class="{"card-vip" if r["vip"] else "card-white"}">{"<div class=\'vip-label\'>⭐ CHOFER VIP</div>" if r["vip"] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛:</b> {r[3]} | 🆔: {cuit} | 📱: {ocultar_telefono(wsp)}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(wsp)}&text=Consulta unidad {r[1]}" target="_blank" class="btn-wsp">✉️ ENVIAR PROPUESTA</a></div>', unsafe_allow_html=True)

# --- TAB 2: CARGAS (CON GENERADOR DE MANIFIESTO) ---
with tab2:
        ...
        if not df_ca_raw.empty:
            ...
            for _, r in df_f2.iterrows():
                ...
                if minutos < TIEMPO_EXCLUSIVO_MIN and not soy_vip_actual:
                    st.markdown(f'<div class="card-bloqueada">🔒 CARGA VIP<br><a href="https://api.whatsapp.com/send?phone={WSP_VENTAS_VIP}" target="_blank" style="color:#f1c40f;">⭐ ACTIVAR VIP</a></div>', unsafe_allow_html=True)
                    ...
                    # Lógica de Manifiesto PDF
                    datos_viaje = f"ORIGEN: {r[1]} | DESTINO: {r[2]} | CARGA: {r[3]} | EMPRESA: {r[5]}"
                    link_manifiesto = f"https://www.google.com/search?q=Manifiesto+Carga+Ignacio+Diaz+{urllib.parse.quote(datos_viaje)}" # Simulación o link a generador
                    
                    st.markdown(f'''
                    <div class="{"card-vip" if r["vip"] else "card-white"}">
                        <span class="dist-badge">{dist}</span>
                        <div class="route-txt">{r[1]} ➔ {r[2]}</div>
                        <b>📦:</b> {r[3]} | 🏢: {r[5]} | 📱: {ocultar_telefono(r[4])}
                        <a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[4])}" target="_blank" class="btn-wsp">📩 CONSULTAR</a>
                        <a href="javascript:window.print()" class="btn-pdf">📄 IMPRIMIR MANIFIESTO DE CARGA</a>
                    </div>
                    ''', unsafe_allow_html=True)
...
with tab3:
    if not df_ca_raw.empty:
        ...
        for _, r in df_arrime.iterrows():
            st.markdown(f'<div class="card-cosecha"><div class="route-txt">📍 {r[2]}</div>{r[3]}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[4])}" target="_blank" class="btn-wsp" style="background:#2e7d32;">🚜 CONTACTAR</a></div>', unsafe_allow_html=True)
