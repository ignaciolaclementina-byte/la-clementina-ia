...
# --- 4. ESTILOS VIP + MEJORA COLORES VENCIMIENTO (IGNACIO DIAZ) ---
st.markdown("""
<style>
    ...
    .new-badge { background: #e74c3c; color: white; padding: 2px 8px; border-radius: 10px; font-size: 10px; font-weight: bold; margin-right: 10px; }
...
def get_card_class(minutos, is_vip_card):
    if is_vip_card: return "card-vip"
...
with st.container():
    ...
    with cstats2: 
        rutas = df_ca_raw[df_ca_raw.iloc[:, 0].apply(lambda x: es_fecha(x, hoy))].iloc[:, 1].nunique() if not df_ca_raw.empty else 0
        st.markdown(f'<div class="stats-card"><span class="stats-val">{rutas}</span><span class="stats-label">Rutas con Carga</span></div>', unsafe_allow_html=True)
...
with st.container():
    ...
    with c_log1:
        ...
        soy_vip_actual = es_vip(user_cuit) if validar_cuit(user_cuit) else False
        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_f.iterrows():
                ...
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])) and (busqueda_libre in str(r).upper()):
                    card_class = get_card_class(minutos, r['vip'])
                    ...
                    link = f"https://api.whatsapp.com/send?phone={limpiar_wsp(wsp)}&text=Consulta por unidad {r[1]} a {r[2]}"
                    st.markdown(f'<div class="{card_class}">{f"<span class=\'dist-badge\'>{dist}</span>" if dist else ""}{"<div class=\'vip-label\'>⭐ CHOFER VIP</div>" if r["vip"] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛 EQUIPO:</b> {r[3]} | 🆔 <b>CUIT:</b> {cuit} | 📱 <b>TEL:</b> {ocultar_telefono(wsp)}<br><a href="{link}" target="_blank" class="btn-wsp">✉️ ENVIAR PROPUESTA</a></div>', unsafe_allow_html=True)
        ...
        if not df_ca_raw.empty:
            ...
            for _, r in df_f2.iterrows():
                ...
                if minutos < TIEMPO_EXCLUSIVO_MIN and not soy_vip_actual:
                    ...
                    st.markdown(f'<div class="{card_class}">{f"<span class=\'dist-badge\'>{dist}</span>" if dist else ""}{"<div class=\'vip-label\'>⭐ EMPRESA VIP</div>" if r["vip"] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>📦 CARGA:</b> {r[3]} | 🏢 <b>EMPRESA:</b> {r[5]} | 📱 <b>TEL:</b> {ocultar_telefono(r[4])}<br><a href="{link}" target="_blank" class="btn-wsp">📩 CONSULTAR</a></div>', unsafe_allow_html=True)
