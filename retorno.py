        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_f.iterrows():
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r[3])):
                    cuit = limpiar_dato(r[4])
                    wsp = limpiar_wsp(r[5])
                    st.markdown(f'<div class="{"card-vip" if r["vip"] else "card-white"}">{"<div class=\"vip-label\">⭐ CHOFER VIP</div>" if r["vip"] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛:</b> {r[3]} | 🆔 <b>CUIT:</b> {cuit}<br><a href="https://api.whatsapp.com/send?phone={wsp}&text=Hola!" target="_blank" class="btn-wsp">💬 WHATSAPP</a></div>', unsafe_allow_html=True)
        ...
        if not df_ca_raw.empty:
            ...
            for _, r in df_f2.iterrows():
                if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r[2]).upper()):
                    st.markdown(f'<div class="{"card-vip" if r["vip"] else "card-white"}">{"<div class=\"vip-label\">⭐ EMPRESA VIP</div>" if r["vip"] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>📦:</b> {r[3]} | 🏢: {r[5]}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[4])}&text=Hola!" target="_blank" class="btn-wsp">💬 CONSULTAR CARGA</a></div>', unsafe_allow_html=True)

# --- PIE DE PÁGINA ---
st.markdown(f"""<div class="legal-footer"><p style="font-size: 18px; font-weight: bold; color: white;">Creado por Ignacio Diaz</p><p style="font-style: italic;">No nos responsabilizamos por acuerdos entre partes.</p><p><b>Prohibida la copia total o parcial sin autorización de Ignacio Diaz.</b></p><p>© 2026 RETORNO MATCH VIP</p></div>""", unsafe_allow_html=True)
