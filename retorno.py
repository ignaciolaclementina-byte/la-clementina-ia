...
def cargar_datos_seguros():
    try:
        t = int(time.time())
        # Cargamos con header=None para evitar el KeyError por nombres de columnas (Blindaje Ignacio Diaz)
...
# Usamos iloc[:, 0] para la fecha ( Blindaje Ignacio Diaz )
...
with tab1:
    ...
    with col_r1:
        if not df_ch_raw.empty:
            # Blindaje .iloc en lambda (Ignacio Diaz)
            ...
            for _, r in df_f.iterrows():
                if len(r) < 6: continue
                minutos_pub = obtener_minutos_desde_publicacion(r.iloc[0])
                badge_nuevo = f'<span class="new-badge">🔥 HACE {int(minutos_pub)} MIN</span>' if minutos_pub < 60 else ""
                distancia = calcular_distancia(str(r.iloc[1]), str(r.iloc[2]))
                if (b_o=="CUALQUIERA" or b_o in str(r.iloc[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r.iloc[2]).upper()) and (b_e=="CUALQUIERA" or b_e==str(r.iloc[3])) and (busqueda_libre in str(r).upper()):
                    val_a, val_b = limpiar_dato_numerico(r.iloc[4]), limpiar_dato_numerico(r.iloc[5])
                    cuit, wsp = (val_a, val_b) if len(val_a) == 11 else (val_b, val_a)
                    link_wsp = f"https://api.whatsapp.com/send?phone={limpiar_wsp(wsp)}&text=Consulta por unidad {r.iloc[1]} a {r.iloc[2]}"
                    card_class = get_card_style(minutos_pub, r['vip'])
                    st.markdown(f'<div class="{card_class}">{f"<span class=\'dist-badge\'>{distancia}</span>" if distancia else ""}{"<div class=\'vip-label\'>⭐ CHOFER VIP</div>" if r["vip"] else ""}<div class="route-txt">{badge_nuevo}{r.iloc[1]} ➔ {r.iloc[2]}</div><b>🚛 EQUIPO:</b> {r.iloc[3]} | 🆔 <b>CUIT:</b> {cuit} | 📱 <b>TEL:</b> {ocultar_telefono(wsp)}<br><a href="{link_wsp}" target="_blank" class="btn-wsp">✉️ ENVIAR PROPUESTA</a></div>', unsafe_allow_html=True)
    ...
    with col_r2:
        if not df_ca_raw.empty:
            ...
            for _, r in df_f2.iterrows():
                ...
                if minutos < TIEMPO_EXCLUSIVO_MIN and not soy_vip_actual:
                ...
                elif (b_o=="CUALQUIERA" or b_o in str(r.iloc[1]).upper()) and (b_d=="CUALQUIERA" or b_d in str(r.iloc[2]).upper()) and (busqueda_libre in str(r).upper()):
                    link_wsp_ca = f"https://api.whatsapp.com/send?phone={limpiar_wsp(r.iloc[4])}&text=Consulta carga {r.iloc[1]} a {r.iloc[2]}"
                    ...
                    st.markdown(f'<div class="{card_class}">{f"<span class=\'dist-badge\'>{distancia}</span>" if distancia else ""}{"<div class=\'vip-label\'>⭐ EMPRESA VIP</div>" if r["vip"] else ""}<div class="route-txt">{badge_nuevo}{r.iloc[1]} ➔ {r.iloc[2]}</div><b>📦 CARGA:</b> {r.iloc[3]} | 🏢 <b>EMPRESA:</b> {r.iloc[5]} | 📱 <b>TEL:</b> {ocultar_telefono(r.iloc[4])}<br><a href="{link_wsp_ca}" target="_blank" class="btn-wsp">📩 CONSULTAR</a><a href="{link_share}" target="_blank" class="btn-share">📢 DIFUNDIR CARGA</a></div>', unsafe_allow_html=True)
                    ...
                    if st.session_state.get('admin_mode', False):
                        if st.button(f"🗑️ BORRAR #{i}", key=f"del_arr_{idx}"):
                            requests.post(URL_CARGAS_POST, data={
                                ...
                                "entry.576675281": f"REF:{r.iloc[0]}", 
