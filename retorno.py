...
# Headers para que Google acepte el envío como si fuera un navegador
...
# --- 2. ESTILOS (ESTRUCTURA ORIGINAL BLINDADA) ---
st.markdown("""
    <style>
    .stApp {
...
with tab_chofer:
    ...
    with col_i:
        ...
        with st.form("form_ch", clear_on_submit=True):
            ...
            if st.form_submit_button("PUBLICAR"):
                ...
                requests.post(URL_CHOFERES_POST, data=data_ch, headers=HEADERS)
                st.success("✅ ¡Enviado al Excel!"); time.sleep(1); st.rerun()
    ...
    with col_d:
        ...
        try:
            ...
            # Mostramos las últimas cargas primero
            for _, r in df_c.iloc[::-1].iterrows():
                ...
                estado_c = str(r[7]).upper().strip()
                es_concretada = "CONCRETADA" in estado_c
                
                # Si está concretada, solo se ve si no hay filtros activos (o podés ocultarlas directamente)
    ...
    with col_a:
        ...
        with st.form("form_em", clear_on_submit=True):
            ...
            if st.form_submit_button("SUBIR CARGA"):
                ...
                resp = requests.post(URL_CARGAS_POST, data=data_em, headers=HEADERS)
