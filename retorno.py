...
# 3. CABECERA Y CONTADORES (Como pediste)
st.markdown("<h1 style='text-align: center; color: white;'>🚛 RETORNO MATCH</h1>", unsafe_allow_html=True)
st.markdown("<h3 style='text-align: center; color: #2ecc71; margin-top: -20px;'>🍎 La Clementina - Logística</h3>", unsafe_allow_html=True)
...
with tab1:
    ...
    for c in st.session_state.cargas:
        if filtro == "Todos" or c['origen'] == filtro:
            ...
            # Formateo de mensaje de WhatsApp (Corregido sin errores de llave)
            msg_wa = f"🍎 *LA CLEMENTINA - RETORNO MATCH*\n\nHola! Vi tu carga de *{c['item']}* en *{c['origen']}*.\n¿Sigue disponible?"
    ...
    if not encontrado:
        st.info("No hay cargas publicadas en esta zona.")
