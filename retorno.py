...
st.markdown(f'<div class="radar-container"><marquee scrollamount="8">🚛 FECHA: {b_fecha.strftime("%d/%m/%Y")} -- ACTIVOS: {cant_camiones} CAMIONES -- ⭐ {st.session_state.anuncios} -- Creado por Ignacio Diaz y sus legales.</marquee></div>', unsafe_allow_html=True)
...
with t1:
    ...
    with col_f1:
        ...
        with st.form("form_carga", clear_on_submit=True):
            ...
            if len("".join(filter(str.isdigit, ew))) >= 10: st.markdown("<p style='color:#25D366;'>✅ WhatsApp Ok</p>", unsafe_allow_html=True)
        ...
        if not df_ch_raw.empty:
            ...
            for _, r in df_final_ch.iterrows():
                if (b_o == "CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d == "CUALQUIERA" or b_d in str(r[2]).upper()) and (b_e == "CUALQUIERA" or b_e == str(r[3])):
                    ...
                    st.markdown(f'''<div class="{clase}">{"<div class='vip-label'>⭐ CHOFER VIP</div>" if r['es_vip'] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>🚛 EQUIPO:</b> {r[3]} | 🆔 <b>ID:</b> {r[4]}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[7])}" target="_blank" class="btn-wsp">💬 CONTACTAR</a></div>''', unsafe_allow_html=True)
...
with t2:
    ...
    with col_f2:
        ...
        with st.form("form_camion", clear_on_submit=True):
            ...
            if len("".join(filter(str.isdigit, cu))) == 11: st.markdown("<p style='color:#25D366;'>✅ CUIT Ok</p>", unsafe_allow_html=True)
            w = st.text_input("WhatsApp (Sin 0 ni 15)", placeholder="Ej: 1122334455")
            if len("".join(filter(str.isdigit, w))) >= 10: st.markdown("<p style='color:#25D366;'>✅ WhatsApp Ok</p>", unsafe_allow_html=True)
        ...
        if not df_ca_raw.empty:
            ...
            for _, r in df_final_ca.iterrows():
                if (b_o == "CUALQUIERA" or b_o in str(r[1]).upper()) and (b_d == "CUALQUIERA" or b_d in str(r[2]).upper()):
                    clase = "card-vip" if r['es_vip'] else "card-white"
                    st.markdown(f'''<div class="{clase}">{"<div class='vip-label'>⭐ EMPRESA VIP</div>" if r['es_vip'] else ""}<div class="route-txt">{r[1]} ➔ {r[2]}</div><b>📦 CARGA:</b> {r[3]} | 🏢 <b>EMPRESA:</b> {r[4]}<br><a href="https://api.whatsapp.com/send?phone={limpiar_wsp(r[6])}" target="_blank" class="btn-wsp">💬 CONSULTAR</a></div>''', unsafe_allow_html=True)

# --- 8. PANEL DE CONTROL (CON GESTIÓN DE BORRADO) ---
st.markdown("---")
with st.expander("⚙️ PANEL DE CONTROL (ADMIN)"):
    ...
    st.session_state.socios_activos = st.text_area("Lista VIP (CUITs autorizados):", st.session_state.socios_activos)
    
    st.markdown("### 🗑️ GESTIÓN DE BASE DE DATOS")
    st.info("Para eliminar usuarios, corregir errores o dar de baja socios, abrí las planillas:")
    c_btn1, c_btn2 = st.columns(2)
    with c_btn1: st.link_button("📂 ELIMINAR CHOFERES", f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/edit#gid={GID_CHOFERES}")
    with c_btn2: st.link_button("📂 ELIMINAR CARGAS", f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/edit#gid={GID_CARGAS}")
    
    if st.button("🚀 GUARDAR Y ACTUALIZAR SISTEMA"): st.rerun()

# --- 9. PIE DE PÁGINA ---
st.markdown(f'<div class="legal-footer"><p style="font-size: 18px; font-weight: bold; color: white;">Creado por Ignacio Diaz y sus legales</p><p>© 2026 RETORNO MATCH VIP - Todos los derechos reservados.</p></div>', unsafe_allow_html=True)
