...
# Orden del Formulario: Origen, Destino, Mercadería, Empresa, Urgencia, WhatsApp
...
def obtener_color_urgencia(estado):
    ...
    if "hoy" in estado: return "#ff4b4b"  # Rojo
    if "mañana" in estado: return "#f1c40f"  # Amarillo
    if "apuro" in estado: return "#2ecc71"  # Verde
    return "#3498db" # Azul (default)
...
st.markdown("""
    ...
    .urgencia-badge { padding: 4px 10px; border-radius: 6px; font-weight: bold; color: white; font-size: 12px; }
...
with t1:
    ...
    with c2:
        ...
        try:
            ...
            df = filtrar_24hs(df)
            if buscar_origen: df = df[df.iloc[:, 1].str.contains(buscar_origen, case=False, na=False)]
            if buscar_destino: df = df[df.iloc[:, 2].str.contains(buscar_destino, case=False, na=False)]
            
            for _, r in df.iloc[::-1].iterrows():
                # Columnas: Fecha, Origen, Destino, Carga, Empresa, Urgencia, Tel
                f, ret, ent, mer, emp, urg, tel = r[0], r[1], r[2], r[3], r[4], r[5], r[6]
                color = obtener_color_urgencia(urg)
                bandera, t_final = detectar_pais_y_whatsapp(tel)
                msg = urllib.parse.quote(f"Hola! Vi tu carga en Retorno Match: {ret} -> {ent} ({mer}). Mi camión está libre. ¿Sigue disponible?")
                    ...
                    <div class="card-white" style="border-left: 10px solid {color};">
                        <div>
                            ...
                            <div style="margin-top:8px; display: flex; flex-wrap: wrap; gap: 10px;">
                                <span class="urgencia-badge" style="background: {color};">{urg}</span>
...
with t2:
    ...
    with c2:
        ...
        try:
            ...
            if buscar_origen: dfh = dfh[dfh.iloc[:, 1].str.contains(buscar_origen, case=False, na=False)]
            if buscar_destino: dfh = dfh[dfh.iloc[:, 2].str.contains(buscar_destino, case=False, na=False)]
...
with st.expander("⚖️ Ver Términos y Condiciones"):
    st.write("Retorno Match es una plataforma de enlace. La propiedad del código pertenece a Ignacio Diaz.")
