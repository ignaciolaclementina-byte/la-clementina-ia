...
<div class="radar-container">
    <marquee scrollamount="8">
        🚛 EN VIVO: {cant_camiones} CAMIONES DISPONIBLES AHORA -- ⭐ {st.session_state.anuncios} -- Creado por Ignacio Diaz y sus legales.
    </marquee>
</div>
""", unsafe_allow_html=True)
    ...
    with col_f1:
        ...
        with st.form("form_carga", clear_on_submit=True):
            ...
            if ew:
                ...
                if len(ew_clean) >= 10: st.markdown("<span style='color:#25D366;'>✅ Formato correcto</span>", unsafe_allow_html=True)
...
with t2:
    ...
    with col_f2:
        ...
        with st.form("form_camion", clear_on_submit=True):
            ...
            cu = st.text_input("CUIT/ID (Sin guiones ni puntos)", placeholder="Ej: 20334445551")
            # Validación visual CUIT
            if cu:
                cu_clean = "".join(filter(str.isdigit, cu))
                if len(cu_clean) == 11: st.markdown("<span style='color:#25D366;'>✅ CUIT correcto</span>", unsafe_allow_html=True)
            
            w = st.text_input("WhatsApp (Sin 0 ni 15)", placeholder="Ej: 1122334455")
            # Validación visual WhatsApp
            if w:
                w_clean = "".join(filter(str.isdigit, w))
                if len(w_clean) >= 10: st.markdown("<span style='color:#25D366;'>✅ Formato correcto</span>", unsafe_allow_html=True)
