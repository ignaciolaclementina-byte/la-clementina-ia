...
# --- 4. CARGA INICIAL Y CONTEO ---
df_ch_raw, df_ca_raw, LISTA_VIPS_GLOBAL = cargar_datos_seguros()
ahora = datetime.now()
hoy = ahora.date()

# Calcular conteos antes de la interfaz
...
<style>
    ...
    .stTabs [data-baseweb="tab"] { flex: 1; height: 70px !important; background-color: #1e272e !important; color: white !important; font-size: 18px !important; font-weight: 900 !important; border-radius: 10px 10px 0 0; margin: 0 2px; }
    ...
    .card-old { background: white !important; border-left: 12px solid #95a5a6 !important; }
    .card-vip { background: #fff9e6 !important; border: 3px solid #f1c40f !important; box-shadow: 0px 4px 15px rgba(241, 196, 15, 0.4); }
    .card-bloqueada { background: #dfe6e9 !important; border: 2px dashed #636e72 !important; padding: 20px; border-radius: 15px; text-align: center; color: #2d3436; }
    .card-white, .card-vip, .card-hot, .card-medium, .card-old { border-radius: 15px; padding: 20px; margin-bottom: 15px; color: #333; }
    ...
    .legal-footer { text-align: center; color: rgba(255,255,255,0.7); padding: 40px 20px; font-size: 14px; border-top: 1px solid rgba(255,255,255,0.2); margin-top: 50px; background: rgba(0,0,0,0.5); }
...
soy_vip_actual = user_cuit.upper() in LISTA_VIPS_GLOBAL

if soy_vip_actual: 
    st.success("🚀 MODO VIP ACTIVADO - Sin esperas")
...
with tab1:
    if not df_ch_raw.empty:
        ...
        for _, r in df_f.iterrows():
            ...
            is_v = str(r[4]).upper() in LISTA_VIPS_GLOBAL or str(r[5]).upper() in LISTA_VIPS_GLOBAL
            if (b_o=="CUALQUIERA" or b_o in str(r[1]).upper()) and (busqueda in str(r).upper()):
                ...
                <div class="{style}">
                    ...
                    <b>EQUIPO:</b> {r[3]} | <b>TEL:</b> {ocultar_telefono(r[5])}<br>
                    <a href="https://wa.me/{limpiar_wsp(r[5])}" class="btn-wsp">✉️ ENVIAR PROPUESTA</a>
                </div>
                ''', unsafe_allow_html=True)
...
with tab2:
    if not df_ca_raw.empty:
        ...
        for _, r in df_f2.iterrows():
            minutos = obtener_minutos_desde_publicacion(r[0])
            is_v = str(r[5]).upper() in LISTA_VIPS_GLOBAL
            
            if minutos < TIEMPO_EXCLUSIVO_MIN and not soy_vip_actual:
                st.markdown(f'<div class="card-bloqueada">🔒 CARGA EXCLUSIVA VIP<br>Disponible en {int(TIEMPO_EXCLUSIVO_MIN-minutos)} min</div>', unsafe_allow_html=True)
                ...
                <div class="{style}">
                    ...
                    <a href="https://wa.me/{limpiar_wsp(r[4])}" class="btn-wsp">📩 CONSULTAR</a>
                    <a href="https://wa.me/?text={txt_share}" class="btn-share">📢 DIFUNDIR</a>
                </div>
                ''', unsafe_allow_html=True)
