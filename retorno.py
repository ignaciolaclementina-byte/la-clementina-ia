...
st.markdown("""
    ...
    .route-txt { font-size: 24px; font-weight: 900; color: #1e3799; text-transform: uppercase; line-height: 1.2; }
    .btn-wsp { 
        background-color: #25D366; color: white !important; padding: 12px; 
        border-radius: 10px; text-decoration: none; font-weight: bold; display: block; text-align: center; font-size: 16px;
    }
...
with c_b1: b_origen = st.text_input("🔍 ORIGEN (Filtrar):").strip()
with c_b2: b_destino = st.text_input("🏁 DESTINO (Filtrar):").strip()
with c_act:
...
# ==========================================
# PESTAÑA 1: SOY CHOFER (Cargar Camión y ver Cargas)
# ==========================================
with tab_chofer:
    ...
    with col_i:
        ...
        with st.form("form_ch", clear_on_submit=True):
            ...
            if st.form_submit_button("PUBLICAR DISPONIBILIDAD"):
                ...
                st.success("✅ Publicado. Verificaremos tus datos."); time.sleep(1); st.rerun()
    ...
    with col_b:
        ...
        try:
            ...
            for _, r in df_h.iloc[::-1].iterrows():
                ...
                # Mapeo según Excel: r[4]:Wsp, r[5]:CUIT, r[6]:LINTI, r[7]:Link, r[8]:Estado
                ...
                msg_em = urllib.parse.quote(
                    ...
                    f"🆔 *CUIT:* {r[5]}\n"
                    f"💳 *LINTI:* {r[6]}\n\n"
                    f"Tengo una carga que te puede interesar. ¿Podemos hablar?"
                )
                ...
                <div class="card-white" style="border-left-color: {color_b};">
                    ...
                    <div style="display:flex; gap:10px; margin-top: 20px;">
                        <a href="https://api.whatsapp.com/send?phone=549{r[4]}&text={msg_em}" target="_blank" class="btn-wsp" style="flex:2; margin-top:0;">💬 HABLAR CON CHOFER</a>
