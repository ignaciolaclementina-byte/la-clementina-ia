...
st.markdown("""
    ...
    label, p, span { color: white !important; font-weight: bold; }
...
# El cargador que ya tenías funcionando
...
if foto:
    ...
    if st.button("🚀 ANALIZAR AHORA"):
        with st.spinner("Conectando con el Ingeniero IA..."):
            try:
                # SOLUCIÓN AL ERROR 404: Usar configuración estable
                genai.configure(api_key=CLAVE)
                # Forzamos el modelo estable sin el prefijo v1beta
                model = genai.GenerativeModel('gemini-1.5-flash')
                
                prompt = f"Actúa como agrónomo. Analiza esta foto de {cul} en {est}. Diagnóstico de plagas. Receta solo productos de esta lista: {VADEMECUM}. Usa 'Dosis: X l/ha'."
                ...
                st.write(f"<span style='color:black;'>{informe}</span>", unsafe_allow_html=True)
                if compra:
                    st.markdown("<hr style='border-top: 2px solid #bbb;'>")
                ...
                st.error(f"Error: {e}. Reintentá en un momento.")
